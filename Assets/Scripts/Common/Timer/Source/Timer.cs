...
namespace Common.Timer.Source
{
    ...
    public class Timer
    {
        ...
        /// <param name="onTimeRemaining">An action that should fire each time the timer is updated. Takes the remaining time</param>
        ...
        public static Timer Register(MonoBehaviour autoDestroyOwner, float duration, Action onComplete, Action<float> onUpdate = null, Action<float> onTimeRemaining = null,
            bool isLooped = false, bool useRealTime = false)
        {
            ...
            Timer timer = new Timer(duration, onComplete, onUpdate, onTimeRemaining, isLooped, useRealTime, autoDestroyOwner);
        ...
        private bool isOwnerDestroyed
        {
            get { return this._hasAutoDestroyOwner && this._autoDestroyOwner == null || 
                         this._hasAutoDestroyOwner && this._autoDestroyOwner.gameObject.activeSelf == false; }
        }

        private readonly Action _onComplete;
        private readonly Action<float> _onUpdate;
        private readonly Action<float> _onTimeRemaining;
        ...
        private Timer(float duration, Action onComplete, Action<float> onUpdate,
            Action<float> onTimeRemaining,bool isLooped, bool usesRealTime, MonoBehaviour autoDestroyOwner)
        {
            this.duration = duration;
            this._onComplete = onComplete;
            this._onUpdate = onUpdate;
            this._onTimeRemaining = onTimeRemaining;
        ...
        private void Update()
        {
            ...
            float timeDelta = worldTime - this._lastUpdateTime;

            if (this.isPaused)
            {
                this._startTime += timeDelta;
            ...
            if (this._onTimeRemaining != null)
            {
                this._onTimeRemaining(this.GetTimeRemaining());
            }

            if (worldTime >= this.GetFireTime())
            {
